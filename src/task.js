class Task {
    constructor(){
        this.nane = 'boi'
        this.domInstanse = document.createElement('div')
        this.topDetails = document.createElement('div')
        this.init()

            
    }

    expand(){
        this.domInstanse.classList.toggle(`expand`)
    }


    init(){
        let self = this                             //WTF 
        this.domInstanse.classList.add('task')
        this.topDetails.classList.add('top-details')
            this.topDetails.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" class="check-icon" viewBox="0 0 24 24" width="24" height="24"><path fill="none" d="M0 0h24v24H0z"/><path d="M12 22C6.477 22 2 17.523 2 12S6.477 2 12 2s10 4.477 10 10-4.477 10-10 10zm0-2a8 8 0 1 0 0-16 8 8 0 0 0 0 16zm-.997-4L6.76 11.757l1.414-1.414 2.829 2.829 5.656-5.657 1.415 1.414L11.003 16z"/></svg>'
            this.topDetails.innerHTML += '<div class="task-title">An Important Thing</div>'

        this.domInstanse.appendChild(this.topDetails)
        this.domInstanse.addEventListener('click', function (){
            self.expand()
        })

    }
}

export {Task}